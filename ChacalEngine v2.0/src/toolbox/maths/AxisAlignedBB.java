...
public class AxisAlignedBB
{
    ...
    public AxisAlignedBB(Vector3 pos)
    {
   ...
   /* public AxisAlignedBB(Vector3 pos1, Vector3 pos2)
    ...
    public AxisAlignedBB offset(Vector3 pos)
    {
    ...
    public AxisAlignedBB multiplySize(float factor)
    {
        return new AxisAlignedBB(this.minX *factor, this.minY *factor, this.minZ *factor, this.maxX *factor, this.maxY *factor, this.maxZ *factor);
    }
    ...
    public double calculateYOffset(AxisAlignedBB other, double offsetY)
    {
        ...
        if (other.maxX > this.minX && other.minX < this.maxX && other.maxZ > this.minZ && other.minZ < this.maxZ)
        {
            ...
            else if (offsetY < 0.0D && other.minY >= this.maxY)
            {
                ...
                { 
                    offsetY = d0;
                   // System.out.println("p2 "+d0+" "+offsetY);
                }
            }
           // System.out.println("return"+offsetY);
            return offsetY;
        }
        else
        {
    ...
    boolean isClosest(Vector3 p_186661_1_, Vector3 p_186661_2_, Vector3 p_186661_3_)
    {
        return p_186661_2_ == null || p_186661_1_.squareDistanceTo(p_186661_3_) < p_186661_1_.squareDistanceTo(p_186661_2_);
    }


    Vector3 collideWithXPlane(double p_186671_1_, Vector3 p_186671_3_, Vector3 p_186671_4_)
    {
    	Vector3 vec3d = p_186671_3_.getIntermediateWithXValue(p_186671_4_, p_186671_1_);
        return vec3d != null && this.intersectsWithYZ(vec3d) ? vec3d : null;
    }


    Vector3 collideWithYPlane(double p_186663_1_, Vector3 p_186663_3_, Vector3 p_186663_4_)
    {
    	Vector3 vec3d = p_186663_3_.getIntermediateWithYValue(p_186663_4_, p_186663_1_);
        return vec3d != null && this.intersectsWithXZ(vec3d) ? vec3d : null;
    }


    Vector3 collideWithZPlane(double p_186665_1_, Vector3 p_186665_3_, Vector3 p_186665_4_)
    {
    	Vector3 vec3d = p_186665_3_.getIntermediateWithZValue(p_186665_4_, p_186665_1_);
        return vec3d != null && this.intersectsWithXY(vec3d) ? vec3d : null;
    }


    public boolean intersectsWithYZ(Vector3 vec)
    {
        return vec.y >= this.minY && vec.y <= this.maxY && vec.z >= this.minZ && vec.z <= this.maxZ;
    }


    public boolean intersectsWithXZ(Vector3 vec)
    {
        return vec.x >= this.minX && vec.x <= this.maxX && vec.z >= this.minZ && vec.z <= this.maxZ;
    }


    public boolean intersectsWithXY(Vector3 vec)
    {
    ...
    public float intersectRay(Vector3 origin, Vector3 direction){
        if(this.isVecInside(origin))return 0;
        float min_distance=Float.POSITIVE_INFINITY;

        if(direction.x!=0){
            double t_x=(maxX-origin.x)/direction.x;
            if(t_x>0) {
                Vector3 intersect = origin.getAdd(direction.getMul((float) t_x));
                if (t_x * direction.length() < min_distance && intersect.y >= minY && intersect.z >= minZ && intersect.y <= maxY && intersect.z <= maxZ)
                    min_distance = (float) t_x * direction.length();
            }
            t_x=(minX-origin.x)/direction.x;
            if(t_x>0) {
                Vector3 intersect=origin.getAdd(direction.getMul((float) t_x));
                if(t_x*direction.length()<min_distance && intersect.y>=minY && intersect.z>=minZ && intersect.y<=maxY && intersect.z<=maxZ)min_distance=(float)t_x*direction.length();
            }

        }
        if(direction.y!=0){
            double t_y=(maxY-origin.y)/direction.y;

            if(t_y>0) {
                Vector3 intersect = origin.getAdd(direction.getMul((float) t_y));
                if(t_y*direction.length()<min_distance && intersect.x>=minX && intersect.z>=minZ && intersect.x<=maxX && intersect.z<=maxZ)min_distance=(float)t_y*direction.length();
            }

            t_y=(minY-origin.y)/direction.y;
            if(t_y>0) {
                Vector3 intersect=origin.getAdd(direction.getMul((float) t_y));
                if(t_y*direction.length()<min_distance && intersect.x>=minX && intersect.z>=minZ && intersect.x<=maxX && intersect.z<=maxZ)min_distance=(float)t_y*direction.length();
            }
        ...
        if(direction.z!=0){
            double t_z=(maxZ-origin.z)/direction.z;
            if(t_z>0) {
                Vector3 intersect=origin.getAdd(direction.getMul((float) t_z));
                if(t_z*direction.length()<min_distance && intersect.x>=minX && intersect.y>=minY && intersect.x<=maxX && intersect.y<=maxY)min_distance=(float)t_z*direction.length();
            }
            t_z=(minZ-origin.z)/direction.z;
            if(t_z>0) {
                Vector3 intersect=origin.getAdd(direction.getMul((float) t_z));
                if(t_z*direction.length()<min_distance && intersect.x>=minX && intersect.y>=minY && intersect.x<=maxX && intersect.y<=maxY)min_distance=(float)t_z*direction.length();
            }
        }
        if(min_distance==Float.POSITIVE_INFINITY)return -1;
        return min_distance;
    }
}
