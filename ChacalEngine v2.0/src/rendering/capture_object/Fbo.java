...
public class Fbo {
	...
	public static final int RG_32=6;
	 ...
	 *            this FBO should use.
	 */
	public Fbo(int width, int height,int colorBufferType, int depthBufferType) {
		this.width = width;
		this.height = height;
		initialiseFrameBuffer(depthBufferType,colorBufferType);
	}
	 ...
	 *            FBO.
	 */
	private void initialiseFrameBuffer(int type_depth,int type_color) {
		//attentiona bien choisir l'attachement
		createFrameBuffer();
		//creation des textures de couleur
		if(type_color==RGBA){
			createTextureAttachment();
		}else if(type_color==RED){
			createRedTextureAttachment();
		}if(type_color==RG_32){
			createRedGreenTextureAttachment();
		}
		//creation des textures de profondeur
		if (type_depth == DEPTH_RENDER_BUFFER) {
			createDepthBufferAttachment();
		} else if (type_depth == DEPTH_TEXTURE) {
			createDepthTextureAttachment();
		}else if(type_depth!=NONE){
			createDepthTextureAttachment();
		}
		unbindFrameBuffer();
	}
	...
	private void createTextureAttachment() {
		...
		if (GL30.glCheckFramebufferStatus(GL30.GL_FRAMEBUFFER) != GL30.GL_FRAMEBUFFER_COMPLETE) {
			throw new RuntimeException("Le framebuffer HDR n'est pas complet !");
		}
	}

	private void createRedTextureAttachment() {
		colourTexture = GL11.glGenTextures();
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, colourTexture);

		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL30.GL_RED, width, height, 0, GL30.GL_RED, GL11.GL_FLOAT,
				(ByteBuffer) null);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL12.GL_CLAMP_TO_EDGE);
		...
		GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, GL11.GL_TEXTURE_2D, colourTexture,
				0);
		if (GL30.glCheckFramebufferStatus(GL30.GL_FRAMEBUFFER) != GL30.GL_FRAMEBUFFER_COMPLETE) {
			throw new RuntimeException("Le framebuffer Red n'est pas complet !");
		}
	}
	private void createRedGreenTextureAttachment() {
		colourTexture = GL11.glGenTextures();
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, colourTexture);

		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL30.GL_RG32F, width, height, 0, GL30.GL_RED, GL11.GL_FLOAT,
				(ByteBuffer) null);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL12.GL_CLAMP_TO_EDGE);
