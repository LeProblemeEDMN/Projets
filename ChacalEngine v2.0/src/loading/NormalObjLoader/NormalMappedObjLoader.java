package loading.NormalObjLoader;
...
public class NormalMappedObjLoader {
	...
	public static RawModel loadOBJ(String objFileName, Loader loader) {
		...
		try {
			...
			while (line != null && line.startsWith("f ")) {
				...
				if(currentLine.length>=5){
					...
					VertexNM v4=processVertex(vertex4, vertices, indices);

					v0=processVertex(vertex1, vertices, indices);
		...
		RawModel model= loader.loadToVOA(verticesArray, texturesArray, normalsArray,tangentsArray, indicesArray);
	...
	private static void calculateTangents(VertexNM v0, VertexNM v1, VertexNM v2,
		...
		Vector3f delatPos1 = new Vector3f(v1.getPosition()).sub(v0.getPosition());
		Vector3f delatPos2 = new Vector3f(v2.getPosition()).sub(v0.getPosition());
		...
		float r = (float)(1.0 / (deltaUv1.x * deltaUv2.y - deltaUv1.y * deltaUv2.x+1e-12));
		if(Float.isNaN(r)) System.out.println(r);
		//attention c'etait scale avant et pas mul a verif si y a un bug.

		delatPos1.mul(deltaUv2.y);
		delatPos2.mul(deltaUv1.y);

		Vector3f tangent = new Vector3f(delatPos1).sub(delatPos2);
