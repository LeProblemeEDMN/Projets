...
public class CubeMapCamera extends Camera {
	...
	private void updateViewMatrix() {
		vMatrix.identity();
		vMatrix.rotate((float) Math.toRadians(180),0, 0, 1);
		vMatrix.rotate((float) Math.toRadians(pitch),1, 0, 0);
		vMatrix.rotate((float) Math.toRadians(yaw), 0, 1, 0);
		Vector3f negativeCameraPos = new Vector3f(-center.x, -center.y, -center.z);
		vMatrix.translate(negativeCameraPos);

		pnMatrix.mul(vMatrix, projectionViewMatrix);
	}
